...
def raccogli(giorni: int = 30) -> Dict[str, Any]:
    """Tutti i numeri della pagina, in una lettura sola del database."""
    ...
    corse = _interroga(
        ...
        f"FROM vista_utilizzo WHERE {filtro}")[0]

    prova = _interroga(
        "SELECT count(*) FROM vista_utilizzo WHERE NOT (" + filtro + ")")[0][0]

    per_giorno = _interroga(
        f"SELECT date_trunc('day', iniziato_il)::date, count(*) "
        f"FROM vista_utilizzo WHERE {filtro} GROUP BY 1")

    per_ora = _interroga(
        f"SELECT EXTRACT(HOUR FROM iniziato_il)::int, count(*) "
        f"FROM vista_utilizzo WHERE {filtro} GROUP BY 1 ORDER BY 1")

    per_mezzo = _interroga(
        f"SELECT mezzo, count(*) FROM vista_utilizzo WHERE {filtro} "
        f"GROUP BY 1 ORDER BY 2 DESC")

    per_chiusura = _interroga(
        f"SELECT chiuso_da, count(*) FROM vista_utilizzo WHERE {filtro} "
        f"GROUP BY 1 ORDER BY 2 DESC")

    posizioni = _interroga(
        f"SELECT lat, lon FROM vista_utilizzo "
        f"WHERE {filtro} AND lat IS NOT NULL")

    def caratteristica(colonna: str) -> List[Tuple]:
        return _interroga(
            f"SELECT {colonna}, count(*) FROM vista_utilizzo WHERE {filtro} "
            f"GROUP BY 1 ORDER BY 2 DESC")
    ...
    voti = _interroga(
        "SELECT rating, count(*) FROM vista_ricerca GROUP BY 1 ORDER BY 1")
    media_voto = _interroga("SELECT avg(rating) FROM vista_ricerca")[0][0]
    commenti = _interroga(
        "SELECT count(*) FROM vista_ricerca WHERE commento IS NOT NULL "
        "AND btrim(commento) <> ''")[0][0]
